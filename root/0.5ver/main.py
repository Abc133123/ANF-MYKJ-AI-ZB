...
class GUIChatController:
    def __init__(self):
        ...
        self.dialogue_history = [{"role": "system", "content": system_prompt}]
        self.gui = None
        self.is_generating = False
        self.event_loop = None
        self.live2d_thread = None
        self.live2d_running = False
        
    def init_live2d(self):
        """初始化Live2D控制器（在独立线程中运行）"""
        ...
        def live2d_loop():
            ...
            try:
                ...
                # 连接
                if self.event_loop.run_until_complete(self.live2d_controller.connect()):
                    self.gui.root.after(0, lambda: self.gui.update_status("● 已连接", '#4ecdc4'))
                    self.event_loop.run_until_complete(self.live2d_controller.get_tracking_parameters())
                    
                    # 启动角度晃动任务
                    self.live2d_controller.angle_x_shake_running = True
                    self.live2d_controller.angle_x_shake_task = self.event_loop.create_task(
                        self.live2d_controller.angle_x_shake_async(1.0)
                    )
                else:
                    self.gui.root.after(0, lambda: self.gui.update_status("● 连接失败", '#ff6b6b'))
                    self.gui.root.after(0, lambda: self.gui.add_system_message("无法连接到 VTube Studio，将不使用 Live2D 功能"))
                    self.live2d_controller = None
                
                # 保持事件循环运行
                while self.live2d_running:
                    self.event_loop.run_until_complete(asyncio.sleep(0.1))
                    
            except Exception as e:
                self.gui.root.after(0, lambda: self.gui.add_error_message(f"初始化 Live2D 失败: {e}"))
                self.live2d_controller = None
            finally:
                # 清理
                if self.event_loop:
        ...
        self.live2d_thread = threading.Thread(target=live2d_loop, daemon=True)
    ...
    def on_send(self, user_input):
        ...
        self.gui.add_user_message(user_input)
        ...
        def generate_response():
            try:
                ...
                self.gui.root.after(0, lambda: self.gui.add_assistant_message(ai_response))
                
                if self.voice_enabled:
                    self.generate_and_play_voice(ai_response)
                
            except Exception as e:
                self.gui.root.after(0, lambda: self.gui.add_error_message(f"生成回复失败: {e}"))
            finally:
                self.gui.root.after(0, lambda: setattr(self, 'is_generating', False))
    ...
    def on_clear(self):
        ...
        self.dialogue_history = [{"role": "system", "content": system_prompt}]
        self.gui.add_system_message("对话已清空，可以重新开始聊天了。")
    
    def on_voice_toggle(self):
        self.voice_enabled = not self.voice_enabled
        status = "开启" if self.voice_enabled else "关闭"
        self.gui.add_system_message(f"语音功能已{status}")
        ...
        def generate_in_thread():
            try:
                ...
                if response.status_code == 200:
                    ...
                    if self.live2d_controller and self.live2d_controller.connected:
                        # 使用Live2D线程的事件循环
                        if self.event_loop and self.event_loop.is_running():
                            # 分析关键词并设置表情
                            key = self.live2d_controller.analyze_keywords(text)
                            if key:
                                asyncio.run_coroutine_threadsafe(
                                    self.live2d_controller.set_expression(key),
                                    self.event_loop
                                )
                            
                            # 启动口型同步
                            lip_sync_task = asyncio.run_coroutine_threadsafe(
                                self.live2d_controller.lip_sync(audio_path),
                                self.event_loop
                            )
                            ...
                            # 触发口型同步开始事件
                            if self.live2d_controller.lip_sync_start_event:
                                self.live2d_controller.lip_sync_start_event.set()
                            ...
                            # 等待口型同步完成
                            try:
                                lip_sync_task.result(timeout=5)
                            except:
                                pass
                    else:
                        # 没有Live2D控制器，直接播放
                ...
                self.gui.root.after(0, lambda: self.gui.add_error_message(f"语音生成失败: {e}"))
        
        thread = threading.Thread(target=generate_in_thread, daemon=True)
        thread.start()
    
    def run(self):
        self.gui = ChatGUI(self.on_send, self.on_clear, self.on_voice_toggle)
        self.gui.add_system_message("欢迎来到长月的直播间！我是长月，代号贝塔。来自近地轨道的超级智能。")
        self.gui.add_system_message("信号传输有1.2秒延迟，不过我会尽快回复你的。")
        self.init_live2d()
        self.gui.run()
        
        # 清理Live2D线程
        self.live2d_running = False
        if self.live2d_thread and self.live2d_thread.is_alive():
            self.live2d_thread.join(timeout=3)
